...
$tipoConsulta = trim((string)($_POST['tipo-consulta'] ?? ''));
...
$tipoConsultaLabel = $tipoConsultaMap[$tipoConsulta] ?? $tipoConsulta;

$subject = 'Nueva consulta desde cetraconsultores.cl';
if ($tipoConsultaLabel !== '') {
  $subject = 'Consulta: ' . $tipoConsultaLabel;
}
...
$tipoConsultaCell = $tipoConsultaLabel !== '' ? $escape($tipoConsultaLabel) : '-';
...
$bodyHtml = '<!doctype html><html><head><meta charset="UTF-8"></head><body style="font-family:Arial,Helvetica,sans-serif; color:#111827;">'
  . '<h2 style="margin:0 0 16px; font-size:18px;">Nueva consulta desde Cetraconsultores</h2>'
  ...
  . '<tr><td style="padding:8px 10px; border:1px solid #E5E7EB; font-weight:700;">Tipo de consulta</td><td style="padding:8px 10px; border:1px solid #E5E7EB;">' . $tipoConsultaCell . '</td></tr>'
...
$bodyText = "Nueva consulta desde Cetraconsultores\n\n"
  . "Nombre: {$nombre}\n"
  . "Email: {$email}\n"
  . "Teléfono: " . ($telefono !== '' ? $telefono : '-') . "\n"
  . "Tipo de consulta: " . ($tipoConsultaLabel !== '' ? $tipoConsultaLabel : '-') . "\n\n"
  . "Mensaje:\n{$mensaje}\n";

$boundary = 'cetracon_' . bin2hex(random_bytes(12));
$body = "--{$boundary}\r\n"
